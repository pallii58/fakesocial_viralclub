...
const LOGO_SCALE: Record<BrandId, number> = {
  instagram: 0.9,
  whatsapp: 0.92,
  facebook: 0.92,
  tiktok: 0.94,
...
function LogoGroup({
  ...
  const s = LOGO_SCALE[brand];
  return (
    <g
      filter={`url(#${id}-glow)`}
      transform={`translate(12 12) scale(${s}) translate(-12 -12)`}
    >
      {children}
    </g>
  );
...
export function NeonBrandLogo({
  ...
  return (
    ...
    >
      ...
      {/* Instagram — logo ufficiale (camera glyph) */}
      {brand === "instagram" && (
        <LogoGroup id={id} brand="instagram">
          <path
            fill={`url(#${id}-ig)`}
      ...
      {brand === "whatsapp" && (
        <LogoGroup id={id} brand="whatsapp">
          <path
            fill={colors.primary}
      ...
      {brand === "facebook" && (
        <LogoGroup id={id} brand="facebook">
          <path
            fill={colors.primary}
